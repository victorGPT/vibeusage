...
test("vibeusage-usage-heatmap returns a week-aligned grid with derived fields", async () => {
  const fn = await loadEdgeFunction("vibeusage-usage-heatmap");
...
test("vibeusage-usage-daily uses hourly when rollup disabled", () =>
  withRollupDisabled(async () => {
    const fn = await loadEdgeFunction("vibeusage-usage-daily");
...
test("vibeusage-usage-summary ignores rollup flag for rolling windows", () =>
  withRollupEnabled(async () => {
    const fn = await loadEdgeFunction("vibeusage-usage-summary");

    const userId = "99999999-9999-9999-9999-999999999998";
    const userJwt = createUserJwt(userId);
    const filters = [];
    ...
    assert.ok(
      filters.some(
        (f) => f.op === "gte" && f.col === "hour_start" && f.value === "2025-12-15T00:00:00.000Z",
      ),
    );
  }));
...
test("vibeusage-usage-summary uses rollup rows for full-day ranges when enabled", () =>
  withRollupEnabled(async () => {
    const fn = await loadEdgeFunction("vibeusage-usage-summary");
    ...
    const rollupRows = Array.from({ length: 30 }, (_, index) => ({
        ...
        return {
          ...
          database: {
            from: (table) => {
              if (table === "vibeusage_tracker_daily_rollup") {
                touched.rollup += 1;
                return createQueryMock({ rows: rollupRows });
              }
              if (table === "vibeusage_tracker_hourly") {
                touched.hourly += 1;
                return createQueryMock({ rows: [] });
    ...
    assert.equal(body.totals.total_tokens, "300");
    assert.equal(body.totals.billable_total_tokens, "300");
...
test(
  ...
  async () => {
    ...
    const dailyFn = await loadEdgeFunction("vibeusage-usage-daily");
    const breakdownFn = await loadEdgeFunction("vibeusage-usage-model-breakdown");
...
test("vibeusage-link-code-init issues a short-lived link code", async () => {
  const fn = await loadEdgeFunction("vibeusage-link-code-init");
