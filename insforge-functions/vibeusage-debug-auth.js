...
var verifyUserJwt2 = ({ token }) => authCore.verifyUserJwt({ token, jwtSecret: getJwtSecret2(), jwksUrl: getJwksUrl() });
...
// insforge-src/functions-esm/vibeusage-debug-auth.js
async function vibeusage_debug_auth_default(request) {
  ...
  const local = await verifyUserJwt2({ token: bearer });
  return json2(
    {
      hasAnonKey: true,
      hasBearer: true,
...
export {
  vibeusage_debug_auth_default as default
};
