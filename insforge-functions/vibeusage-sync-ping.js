...
var CORE_KEY9 = "__vibeusageCryptoCore";
...
var vibeusage_sync_ping_default = withRequestLogging2("vibeusage-sync-ping", async function(request, logger) {
  ...
  if (!serviceRoleKey && !anonKey) {
    return json2({ error: "Missing anon key" }, 500);
  }
  ...
  if (serviceRoleKey) {
    ...
    if (tokenErr) return json2({ error: tokenErr.message }, 500);
    if (!tokenRow || tokenRow.revoked_at) return json2({ error: "Unauthorized" }, 401);
    const lastSyncAt = normalizeIso2(tokenRow.last_sync_at);
    if (lastSyncAt && isWithinInterval2(lastSyncAt, MIN_INTERVAL_MINUTES)) {
      return json2(
        {
          success: true,
          updated: false,
    ...
    if (updateErr) return json2({ error: updateErr.message }, 500);
    return json2(
      {
        success: true,
        updated: true,
  ...
  try {
    ...
    if (!touch) return json2({ error: "Unauthorized" }, 401);
    return json2(
      {
        success: true,
...
export {
  vibeusage_sync_ping_default as default
};
