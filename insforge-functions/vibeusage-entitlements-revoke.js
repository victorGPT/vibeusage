...
var vibeusage_entitlements_revoke_default = withRequestLogging2("vibeusage-entitlements-revoke", async function(request) {
  ...
  if (!id) return json2({ error: "id is required" }, 400);
  if (revokedAt && !isValidIso(revokedAt)) {
    return json2({ error: "revoked_at must be ISO timestamp" }, 400);
  }
  ...
  return json2({ id, revoked_at: update.revoked_at }, 200);
});
...
export {
  vibeusage_entitlements_revoke_default as default
};
