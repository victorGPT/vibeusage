...
var vibeusage_viewer_identity_default = withRequestLogging2("vibeusage-viewer-identity", async function(request) {
  ...
  const access = await getAccessContext2({ baseUrl, bearer, allowPublic: false });
  if (!access.ok) {
    return json2({ error: access.error || "Unauthorized" }, access.status || 401);
  }
  ...
  if (error) {
    return json2({ error: "Failed to fetch viewer identity" }, 500);
  }
