...
var CORE_KEY8 = "__vibeusageProStatusCore";
...
function normalizeSubscriptions(rows) {
  ...
  for (const row of list) {
    ...
    out.push({
      ...
      active_start: normalizeIso2(row?.active_start),
      active_until: normalizeIso2(row?.active_until),
      last_checked: normalizeIso2(row?.last_checked),
      observed_at: normalizeIso2(row?.observed_at),
      updated_at: normalizeIso2(row?.updated_at)
    });
  }
  return out;
}
...
function getLatestIso(rows, key) {
  const list = Array.isArray(rows) ? rows : [];
  for (const row of list) {
    const iso = normalizeIso2(row?.[key]);
    if (iso) return iso;
  }
  return null;
}
