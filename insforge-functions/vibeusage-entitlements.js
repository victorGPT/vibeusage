...
var CORE_KEY7 = "__vibeusageLoggingCore";
...
var CORE_KEY8 = "__vibeusageCryptoCore";
...
var vibeusage_entitlements_default = withRequestLogging2("vibeusage-entitlements", async function(request) {
  ...
  const isProjectAdmin = isProjectAdminBearer2(bearer);
  if (!isServiceRole && !isProjectAdmin) return json2({ error: "Unauthorized" }, 401);
  ...
  if (!rawUserId) return json2({ error: "user_id is required" }, 400);
  if (!userId) return json2({ error: "user_id must be a UUID" }, 400);
  if (!source || !ALLOWED_SOURCES.has(source)) return json2({ error: "invalid source" }, 400);
  if (data.id != null && !providedId) return json2({ error: "id must be a UUID" }, 400);
  if (data.idempotency_key != null && !idempotencyKey) {
    return json2({ error: "idempotency_key must be a non-empty string" }, 400);
  }
  if (!isValidIso(effectiveFrom) || !isValidIso(effectiveTo)) {
    return json2({ error: "effective_from/effective_to must be ISO timestamps" }, 400);
  }
  if (Date.parse(effectiveFrom) >= Date.parse(effectiveTo)) {
    return json2({ error: "effective_to must be after effective_from" }, 400);
  }
  const anonKey = getAnonKey2();
  if (!anonKey && !serviceRoleKey) return json2({ error: "Admin key missing" }, 500);
  ...
  if (entitlementId) {
    ...
    if (existing.error) return json2({ error: existing.error }, 500);
    if (existing.row) {
      if (!matchesEntitlementPayload(existing.row, payload)) {
        return json2({ error: "Entitlement already exists with different payload" }, 409);
      }
      return json2(existing.row, 200);
    }
  }
  ...
  if (!insertError) return json2(row, 200);
  if (entitlementId) {
    const existing = await loadEntitlementById({ dbClient, id: entitlementId });
    if (existing.error) return json2({ error: existing.error }, 500);
    if (existing.row) {
  ...
  return json2({ error: insertError.message || "Insert failed" }, 500);
});
...
async function resolveEntitlementId({ userId, providedId, idempotencyKey }) {
  ...
  const hash = await sha256Hex2(`${userId}:${idempotencyKey}`);
...
export {
  vibeusage_entitlements_default as default
};
