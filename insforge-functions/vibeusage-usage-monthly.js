...
var CORE_KEY9 = "__vibeusageUsageFilterRequestCore";
...
var CORE_KEY11 = "__vibeusageCanaryCore";
...
var CORE_KEY12 = "__vibeusagePaginationCore";
...
var CORE_KEY13 = "__vibeusageUsageFilterCore";
...
var CORE_KEY14 = "__vibeusageUsageHourlyQueryCore";
...
function resolveHourlyUsageRowState({
  ...
  const sourceKey = runtimePrimitivesCore4.normalizeSource(row?.source) || source || defaultSource;
...
var CORE_KEY17 = "__vibeusageUsageRowCollectorCore";
...
var CORE_KEY18 = "__vibeusageDateCore";
...
var toBigInt2 = runtimePrimitivesCore5.toBigInt;
var toPositiveIntOrNull2 = runtimePrimitivesCore5.toPositiveIntOrNull;
var toPositiveInt2 = runtimePrimitivesCore5.toPositiveInt;

// insforge-src/shared/usage-monthly-core.mjs
var CORE_KEY19 = "__vibeusageUsageMonthlyCore";
...
function ingestMonthlyRow({
  ...
  bucket.billable += toBigInt3(usageRow?.billable);
...
// insforge-src/functions-esm/shared/core/usage-monthly.js
var usageMonthlyCore = globalThis.__vibeusageUsageMonthlyCore;
if (!usageMonthlyCore) throw new Error("usage monthly core not initialized");
var initMonthlyBuckets2 = usageMonthlyCore.initMonthlyBuckets;
...
var vibeusage_usage_monthly_default = withRequestLogging2("vibeusage-usage-monthly", async function(request, logger) {
  ...
  const monthsParsed = toPositiveIntOrNull2(monthsRaw);
  ...
  const toParts = toRaw ? parseDateParts2(toRaw) : { year: todayParts.year, month: todayParts.month, day: todayParts.day };
  if (!toParts) return respond({ error: "Invalid to date" }, 400, 0);
  const startMonthParts = addDatePartsMonths2(
    { year: toParts.year, month: toParts.month, day: 1 },
    -(months - 1)
  );
  const from = formatDateParts2(startMonthParts);
  const to = formatDateParts2(toParts);
  if (!from || !to) return respond({ error: "Invalid to date" }, 400, 0);
  const startUtc = localDatePartsToUtc2(
  ...
  const monthly = monthKeys.map((key) => {
    ...
    return {
      month: key,
      ...usageMetricsCore2.buildUsageBucketPayload(bucket)
    };
  });
  return respond({ from, to, months, data: monthly }, 200, queryDurationMs);
});
export {
  vibeusage_usage_monthly_default as default
};
