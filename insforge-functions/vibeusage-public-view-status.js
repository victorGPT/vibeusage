...
var vibeusage_public_view_status_default = withRequestLogging2("vibeusage-public-view-status", async function(request) {
  ...
  const auth = await getEdgeClientAndUserId2({ baseUrl: getBaseUrl2(), bearer });
  ...
  if (settingsErr) return json2({ error: "Failed to fetch public view status" }, 500);
  if (settings?.leaderboard_public !== true) return json2({ enabled: false }, 200);
  ...
  if (error) return json2({ error: "Failed to fetch public view status" }, 500);
  return json2({ enabled: Boolean(data && !data.revoked_at) }, 200);
});
export {
  vibeusage_public_view_status_default as default
};
