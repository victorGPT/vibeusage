...
var getEdgeClientAndUserId2 = ({ baseUrl, bearer }) => authCore.getEdgeClientAndUserId({
  baseUrl,
  bearer,
...
async function vibeusage_leaderboard_profile_default(request) {
  ...
  if (!isSelf) {
    ...
    if (activeLinkErr) return json2({ error: "Failed to resolve public share state" }, 500);
    if (!activeLink?.user_id) return json2({ error: "Not found" }, 404);
  }
  ...
  return json2(
    {
      ...
      from,
      to,
      generated_at: normalizeLeaderboardGeneratedAt2(snapshot.generated_at),
...
function normalizeSnapshotEntry(row) {
  const displayName = normalizeLeaderboardDisplayName2(row?.display_name);
  const avatarUrl = normalizeLeaderboardAvatarUrl2(row?.avatar_url);
  ...
  const otherTokens = resolveLeaderboardOtherTokens2({
    row,
    totalTokens,
    gptTokens,
    claudeTokens
  ...
  return {
    ...
    rank: toPositiveIntOrNull2(row?.rank),
