...
async function getAccessContext({
  ...
  const publicView = typeof resolvePublicView4 === "function" ? await resolvePublicView4({ baseUrl, shareToken: bearer }) : null;
  if (!publicView?.ok) {
    return {
      ok: false,
      edgeClient: null,
...
var resolvePublicView3 = resolvePublicView2;
...
var vibeusage_public_view_profile_default = withRequestLogging2("vibeusage-public-view-profile", async function(request) {
  ...
  const publicView = await resolvePublicView3({ baseUrl, shareToken: bearer });
  if (!publicView.ok) return json2({ error: "Unauthorized" }, 401);
  ...
  if (error) return json2({ error: "Failed to fetch public profile" }, 500);
  const { displayName, avatarUrl } = resolveUserIdentity(data);
  return json2({ display_name: displayName, avatar_url: avatarUrl }, 200);
});
export {
