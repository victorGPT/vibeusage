...
- `VIBEUSAGE_HTTP_TIMEOUT_MS`：可选；HTTP 请求超时（毫秒）。`0` 表示关闭超时；默认 `20000`；取值会被限制在 `1000..120000`
- `VIBEUSAGE_DEBUG`：可选；`1` 时输出请求/响应与原始错误信息（仅 stderr）
- `VIBEUSAGE_AUTO_RETRY_NO_SPAWN`：可选；`1` 时不启动自动重试进程
